...
const testDataTable1 = buildTestDataTable1(testData);
...
test('Reseller Management', async ({ page }) => {
  const user = generateUserName();
  const user1 = generateUserName();
  const address = generateAddress();
  const address1 = generateAddress();
  ...
  const testData = {
  'Reseller Name': user.firstName, 'Email': user.email, 'Billing Code': user.billingcode, 'Phone': user.phoneNumber, 'First Name': user.firstName, 'Middle Name': user.middleName, 'Last Name': user.lastName, 'Street': address.street, 'Postal Code': address.Postalcode, 'City': address.city, 'Country': address.country || 'India', };

  console.log('--- ADD RESELLER DATA ---');
  const testDataTable = buildTestDataTable(testData);
  console.log(testDataTable);
...
console.log('--- EDIT RESELLER DATA ---');
...
console.log(buildTestDataTable(editClientData));
  ...
  await page.getByRole('textbox', { name: 'Telephone number' }).first().click();
  await page.getByRole('textbox', { name: 'Telephone number' }).first().fill(user.phoneNumber);
  await page.getByText('Reseller').click();
  await page.getByRole('textbox', { name: 'First name' }).fill(user.firstName);
  await page.getByRole('textbox', { name: 'Middle name' }).click();
  await page.getByRole('textbox', { name: 'Middle name' }).fill(user.middleName);
  await page.getByRole('textbox', { name: 'Last name' }).click();
